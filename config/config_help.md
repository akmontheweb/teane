...
Spend caps. hard_cap_usd is the per-session dollar ceiling — the gateway refuses dispatches once spend reaches it. Override per-run via --budget. context_window_threshold_pct triggers context compaction when the message list fills this fraction of the model's window. stages (optional) is a soft per-stage budget split — maps NodeRole name to a fraction of hard_cap_usd, e.g. {"planning":0.2,"patching":0.2,"repair":0.5,"doc_reviewer":0.05,"code_reviewer":0.05}. Today this only emits a warning when a stage exceeds its share; hard enforcement is a follow-up. default_hard_cap_usd is the fallback total budget used only when hard_cap_usd is absent (keeps the fallback consistent — it previously diverged across ~8 call sites). synthesis_envelope_usd bounds the doc/spec synthesis calls (requirements / architecture / refine) and installation_doc_floor_usd the terminal INSTALLATION.md synth when the run's real remaining budget isn't threaded; fanout_default_budget_usd is the fanout tool's default $ budget. gates is a nested dict of budget-enforcement thresholds: force_local_below_usd (switch to the local model when remaining drops below this), refuse_if_est_cost_fraction_over (warn/refuse a dispatch whose pre-flight estimate exceeds this fraction of remaining), low_budget_skip_usd (skip optional spec/architecture review cycles and low-value node work below this), and min_call_budget_usd (minimum remaining required to attempt a call). All default to their historical literals; every budget cap is now operator-controlled here rather than hardcoded.
...
ADR-0006 build-time acceptance-criterion verification. Phase 0 ships the dual-altitude LLM scenario/seed generators (`harness/acceptance_gen.py`); the in-build `acceptance_node`, the deferral state machine, and enforcement land in later phases. The generators produce, per acceptance criterion, an `integration`-altitude scenario (pytest against the app's in-process `client` fixture — runnable in the build sandbox, no browser/deploy) for `backend-verifiable` criteria, and an `e2e`-altitude Playwright scenario for every criterion; `ui-only` criteria get only the e2e scenario and are deferred to the post-deploy `teane test` browser pass. Generated scenarios are validated to reject rubber-stamps — a missing/weak/tautological assertion or the `toHaveTitle(/.+/)` placeholder is dropped, not shipped.

**`enabled`** (default **true** — enabled after live validation on lumina) — master switch for in-build acceptance verification. Requires `scenario_source=llm` to be meaningful (with `fallback`, the scaffold scenarios are designed to fail). Set false to disable. **`scenario_source`** (`llm` | `fallback`, default **`llm`**) / **`seed_source`** (`llm` | `fallback`, default `fallback`) — `llm` uses the gateway-backed generator (the `planning` model role); `fallback` uses the offline deterministic scaffold (honest, designed-to-fail TODO bodies — never a green tautology). Note: integration scenarios are generated *self-contained* (each test arranges its own state via the API) and run against a *fresh isolated DB* per test, so `seed_source=llm` is generally counterproductive for the integration altitude — pre-seeded rows break tests that assert empty/known-count state (a validated finding). Keep `seed_source=fallback` for integration; seed data is intended for the Phase-3 browser/e2e altitude. The `llm` seed generator grounds on the workspace's actual `CREATE TABLE` DDL (not a spec doc, which may be absent) and tags rows with `_verifies` for the criteria they support. **`altitudes`** (default `["integration", "e2e"]`) — which altitudes to emit; drop `"e2e"` to generate only the in-build integration tests. **`max_scenarios_per_story`** (default 24) — caps scenarios emitted per story so a large-AC story can't blow the budget. **`max_seed_rows_per_table`** (default 20) — caps seed rows per table. **`integration_dir`** (default `tests/acceptance`) — where rendered integration pytest modules are written. **`max_repair_rounds_per_batch`** (default 2) — Phase-1 `acceptance_node`: how many repair rounds an attributable acceptance failure may drive per batch before the residual failures downgrade to a parked defect and the batch seals `complete_with_blocks` (the finsearch-156032347 no-infinite-loop guard). Deferred (dependency-blocked / ui-only) ACs never consume this budget — they re-queue. **`db_path_env`** (default `""`) — the environment variable that overrides the app's database path, used by the generated acceptance conftest to give each test a fresh isolated SQLite DB (so mutation/empty-state criteria verify deterministically). Empty → auto-discover from a pydantic-settings `env_prefix` + db-path field (e.g. `LUMINA_DB_PATH`); set explicitly to override discovery. When neither is found, isolation is skipped and self-contained scenarios carry verification.

## `decomposition`

ADR-0007 decomposition-quality review — judges the *craftsmanship* of the generated stories/ACs (story sizing/atomicity, overlap, AC testability/atomicity/ambiguity, dependency correctness, over-/under-decomposition), complementing `spec_reconciler` (structural) and `traceability.semantic_review` (coverage). Two tiers: a deterministic gate (dangling/circular deps, zero-AC stories, duplicate stories, empty ACs — provable, no LLM) plus an adversarial LLM pass on the `decomposition_reviewer` model (independent of the decomposer; recommend routing it `thinking`). Runs after reconciliation, before the STORIES gate; writes `docs/DECOMPOSITION_REVIEW.md`.

**`quality_review`** (default **true** — advisory) — master switch; pass-through no-op when off. **`quality_enforce`** (default false) — block the build (exit_code=1, one-shot) on HIGH-severity findings only; otherwise advisory (logs + `decomposition_quality_findings` event + the review doc). Low/medium findings never block, so a subjective call can't stall a headless run. **`max_stories_per_review`** (default 60) — caps how many stories are sent to the LLM reviewer in one prompt.

## `traceability`

End-of-run traceability gate (v5 SQL audit over state.db). When enforce=true (default) the session blocks with exit_code=1 if any spec requirement has no satisfying story (an untraced requirement) — and, in the `test` flow ONLY, if any acceptance criterion has no verifying test. enforce_reqs / enforce_acs override each dimension independently and default to the coarse `enforce` value; AC enforcement is ADDITIONALLY always hard-gated to the `test` flow in code, because build/patch flows generate code-linked unit tests, not AC-linked Playwright tests (blocking them on AC gaps produced an unfixable headless auto-resume loop, finsearch 156032347). Set enforce=false (or enforce_reqs=false) to downgrade the block to an advisory that still prints the full report but ships anyway — use when a spec has known-uncovered requirements you are consciously accepting. The block message and route_after_installation_doc cap (TRACEABILITY_BLOCK_CYCLE_CAP=2) exist so a headless run fires HITL once then exits cleanly instead of ping-ponging (ciod 523e86a7 burned 376 iterations here). Note: per-workspace .harness_config.json is IGNORED — this canonical config is the only place these knobs are read.

Coverage rolls UP the requirement hierarchy: a feature/epic counts as covered when a descendant story covers it (parsed from the requirements' `**Parent feature:**` / `**Parent epic:**` markers), so a feature whose child stories are present is not falsely reported untraced. The requirement-coverage check ALSO runs right after decomposition+reconcile (not only end-of-run), so a genuine gap fails fast before the whole build budget is spent.

**`gap_fill`** (default true) — automated requirement gap-fill: when a FEATURE has no covering story, an LLM (the `decomposition` role) drafts one, an optional reviewer (`decomposition_reviewer` role) vets it, and it's appended to `docs/SPEC_REQUIREMENTS.md` and re-reconciled, up to `node_throttle.max_requirement_gap_fill_cycles` (default 2) before falling through to the fail-fast END. Set false to disable auto-remediation and fail fast immediately.

**`semantic_review`** (default false) — adversarial semantic-coverage review: after coverage is deterministically complete, the `decomposition_reviewer` model judges whether each feature's stories (their acceptance criteria) actually SATISFY the feature's intent, not just cite it. Advisory by default (logs findings + emits a `semantic_coverage_findings` event). **`semantic_review_enforce`** (default false) — block the build (exit_code=1) on a partial/unsatisfied finding instead of only warning. Requires a `decomposition_reviewer_primary` model; skipped when none is configured.

## `planning`

Decomposition-time planning behaviour (ADR-0004). **`embed_constraint_nfrs`** (default true) — fold *constraint-class* NFR stories into the functional stories they constrain instead of scheduling them as separate `STORY-NFR-N` work that runs after the code is already built. A constraint NFR (input sanitisation, validation, error-shaping, encoding — classified deterministically then LLM-refined at requirements-refinement) has its acceptance criteria appended (attributed to its shared policy, prefixed `[NFR:<policy-id>] `) and its `requirement_keys` merged into every functional story whose `scope_files` overlap it, and the separate NFR story is dropped; so the constraint is built and verified in the same patching/repair pass and stays linked to the AC-coverage gate. The authoritative policy definition is injected into the per-story patch prompt so the rule is implemented consistently wherever it applies. *Capability* NFRs (offline mode, load budget, durability, observability, rate-limiting) are always kept as separate enabler stories. Fail-safe: a constraint NFR that matches no functional story (empty/non-overlapping scope) is KEPT — coverage is never silently dropped (the lumina 019fa046 failure this prevents). Set false to revert to separate NFR stories.
