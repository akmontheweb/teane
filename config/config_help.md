...
Decomposition-time planning behaviour (ADR-0004). **`embed_constraint_nfrs`** (default false) — fold *constraint-class* NFR stories into the functional stories they constrain instead of scheduling them as separate `STORY-NFR-N` work that runs after the code is already built. A constraint NFR (input sanitisation, validation, error-shaping, encoding — classified by `_classify_nfr`) has its acceptance criteria appended (prefixed `[NFR] `) and its `requirement_keys` merged into every functional story whose `scope_files` overlap it, and the separate NFR story is dropped; so the constraint is built and verified in the same patching/repair pass and stays linked to the AC-coverage gate. *Capability* NFRs (offline mode, load budget, durability, observability, rate-limiting) are always kept as separate enabler stories. Fail-safe: a constraint NFR that matches no functional story (empty/non-overlapping scope) is KEPT — coverage is never silently dropped (the lumina 019fa046 failure this prevents). Off by default while the change is validated against real rebuilds; flip to true to opt in.
