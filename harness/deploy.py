...
def _generate_compose_file(
    ...
    *,
    ...
    for svc_name, svc_spec in services.items():
        ...
        # Run build-context services as the host user so bind-mounted
        # workspace files don't get chowned to root. DB / proxy images
        # keep their own uid — they use named volumes, not bind mounts.
        if has_build_context:
            lines.append('    user: "${UID:-1000}:${GID:-1000}"')
        ...
        # Suppress bytecode / warning writes that would land on the host
        # bind-mount. PYTHONDONTWRITEBYTECODE is a no-op for non-Python
        # services; NODE_NO_WARNINGS likewise for non-Node.
        env_keys = list(svc_spec.get("environment_keys_needed") or [])
        if has_build_context or env_keys:
            lines.append("    environment:")
            if has_build_context:
                lines.append("      - PYTHONDONTWRITEBYTECODE=1")
                lines.append("      - NODE_NO_WARNINGS=1")
            for key in env_keys:
...
def _generate_env_file(blueprint: dict[str, Any]) -> str:
    ...
    Also writes UID / GID captured from the invoking harness process
    so the ``user: "${UID:-1000}:${GID:-1000}"`` substitution in each
    build-context service resolves to the *actual* host user. Without
    this, compose can't see the shell's UID (bash doesn't export it)
    and would fall back to 1000 — which is only correct by accident.
    """
    ...
    for svc_spec in services.values():
        ...
        for key in svc_spec.get("environment_keys_needed", []) or []:
            if isinstance(key, str) and key not in seen:
                seen.add(key)
                keys.append(key)
    has_build_service = any(
        isinstance(s, dict) and s.get("build_context")
        for s in services.values()
    )
    if not keys and not has_build_service:
        return ""
    lines = [
