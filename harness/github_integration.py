...
def create_pr(
    ...
    *,
    ...
    # Audit §2.16: ARG_MAX is ~128 KB on Linux. Autogenerated PR bodies
    # (security review summaries, fan-out audit output, etc.) can easily
    # exceed that and hit E2BIG. Pipe the body via stdin with --body-file -
    # when it's larger than a conservative 64 KB; argv path is kept for
    # smaller bodies so the existing test stubs don't have to mock stdin.
    body_text = body or ""
    use_stdin_body = len(body_text.encode("utf-8", errors="replace")) > 65536
    cmd = [
        binary, "pr", "create",
        "--title", title,
        "--base", base,
