            ...
            if actual == expected:
                ...
                return PatchResult(
                    ...
                    error=(
                        ...
                        "on disk. Your patch changed nothing. Work through "
                        "these in order — the first two are checkable "
                        "against the file itself, the third is a guess:\n"
                        "  1) You re-emitted the file's CURRENT content — "
                        "your mental model of what it should contain is out "
                        "of date, or you meant to change something and "
                        "didn't. READ_FILE this file, decide the concrete "
                        "edit, and emit content that DIFFERS.\n"
                        "  2) This file is itself the regression. If it was "
                        "modified earlier this session (check its "
                        "modification history in the workspace context) and "
                        "the build broke after that, the fix is to change it "
                        "BACK, not to leave it alone.\n"
                        "  3) Only if neither holds: the file really is "
                        "correct and the bug is somewhere ELSE (a caller, an "
                        "import site, a test expectation). Re-read the "
                        "judge's real_blocker and pick a different target.\n"
        ...
        if search and _is_identity_replacement(search, replace):
            ...
            return PatchResult(
                ...
                error=(
                    ...
                    "nothing. Emitting it burned the round. Work through "
                    "these in order — the first two are checkable against "
                    "the file itself, the third is a guess:\n"
                    "  1) You intended an edit but copied the original text "
                    "into `replace`. Re-emit with the CHANGED text in "
                    "`replace`.\n"
                    "  2) This file is itself the regression. If it was "
                    "modified earlier this session (check its modification "
                    "history in the workspace context) and the build broke "
                    "after that, the fix is to change it BACK, not to leave "
                    "it alone.\n"
                    "  3) Only if neither holds: the file really is correct "
                    "and the bug is somewhere ELSE (a caller, an import "
                    "site, a test expectation, a config/manifest). Re-read "
                    "the judge's real_blocker and pick a different target.\n"
